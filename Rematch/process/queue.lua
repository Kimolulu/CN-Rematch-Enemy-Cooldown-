...
function rematch.queue:InsertPetID(petID,index)
    assert(type(index)=="number" and index>=1 and index<=(#settings.LevelingQueue+1),"队列中无效的索引("..(index or "nil").."):InsertPetID(index)")
...
function rematch.queue:MoveIndex(oldIndex,newIndex)
    ...
    assert(type(oldIndex)=="number" and oldIndex>=1 and oldIndex<=queueSize,"队列中无效的oldIndex("..(oldIndex or "nil").."):MoveIndex(oldIndex,newIndex)")
    assert(type(newIndex)=="number" and newIndex>=1 and newIndex<=(queueSize+1),"队列中无效的("..(newIndex or "nil").."):MoveIndex(oldIndex,newIndex)")
