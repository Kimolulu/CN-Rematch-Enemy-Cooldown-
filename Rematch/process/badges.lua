...
function rematch.badges:RegisterBadge(list,name,icon,coords,callback)
    assert(type(list)=="string" and list:len()>0,"无效的列表名字: "..(list or "nil"))
    assert(type(name)=="string" and name:len()>0,"无效的徽章名称: "..(name or "nil"))
    assert(type(callback)=="function","无效的回调函数用于"..name.."徽章")
