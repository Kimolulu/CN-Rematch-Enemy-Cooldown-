...
function rematch.dialog:Register(name,info)
    assert(type(name)=="string","对话框"..(name or "nil").."的名称无效")
    assert(type(info)=="table","对话框"..name.."没有信息表")
    info.name = name
    ...
    if info.layout then
        assert(type(select(2,next(info.layout)))~="table","默认布局为"..name.."是一个嵌套表. 是否打算使用布局?")
    ...
    if info.layouts then
        for layoutName,layout in pairs(info.layouts) do
            for _,parentKey in ipairs(layout) do
                assert(self.Canvas[parentKey],"对话框布局"..layoutName.."对于"..name.."有一个无效的控件"..parentKey)
            end
        end
    end
end
...
function rematch.dialog:ShowDialog(name,subject,layoutTab)
    ...
    assert(info,"名为"..(name or "nil").."的对话框不存在.")
...
function applyLayout(self,layoutName)
    ...
    assert(type(layout)=="table","布局"..(layoutName or "nil").. "对于"..info.name.."不存在.")
