...
function RematchDialogLayoutTabsMixin:SetTabs(layouts)
    assert(type(layouts)=="table" and type(layouts[1])=="table" and #layouts[1]>=2,"无效的LayoutTabs. 必须是{{tab_name, layout_name}, {tab_name, layout_name}等等.}")
...
function RematchDialogTeamPickerMixin:SetList(listType,teamList)
    assert(listType==C.LIST_TYPE_TEAM or listType==C.LIST_TYPE_TARGET or listType==C.LIST_TYPE_GROUP,"对话框团队列表的无效列表类型")
    assert(type(teamList)=="table","对话框团队列表的无效列表")
