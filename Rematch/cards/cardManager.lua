...
function rematch.cardManager:Register(cardname,frame,def)
    ...
    assert(type(cardname)=="string" and cardname:len()>0,"无效的卡片cardname: '"..(cardname or "nil"))
    assert(not cardInfo[frame],"卡片"..cardname.."的框架已被注册.")
    assert(type(def.update)=="function","尝试注册缺少更新函数的卡片管理器.")
    for _,info in pairs(cardInfo) do
        assert(info.cardname~=cardname,"卡片"..cardname.."已被注册.")
    end
