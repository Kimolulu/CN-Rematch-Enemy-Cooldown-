...
function RematchAutoScrollBoxMixin:Setup(definition)
    ...
    assert(type(definition)=="table","无效的AutoScrollBox定义表")
    assert(type(definition.allData)=="table","缺少allData的AutoScrollBox")
    assert(type(definition.normalTemplate)=="string" and type(definition.normalFill)=="function","无效的AutoScrollBox常规模板")
    assert(type(definition.normalHeight)=="number" and definition.normalHeight>0,"无效的AutoScrollBox常规高度")
...
function RematchAutoScrollBoxMixin:SetupSelect(name,def)
    assert(type(name)=="string" and type(def)=="table","无效的AutoScrollBox SetupSelect")
