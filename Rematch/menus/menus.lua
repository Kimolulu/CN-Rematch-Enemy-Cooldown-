...
function rematch.menus:Register(menuName,menuTable,openFunc,closeFunc)
    assert(menuName,"menuName是nil")
    assert(type(menuTable)=="table","menuTable对于"..menuName.."不是一个有序表.")
...
function rematch.menus:AddToMenu(menuName,menuItem,afterText)
    assert(menuName and allMenus[menuName],"菜单定义不存在于"..(menuName or "nil"))
    assert(type(menuItem)=="table" and menuItem.text,"在AddToMenu中无效的menuItem")
...
function rematch.menus:Show(menuName,parent,subject,...)
    assert(menuName,"menuName是'nil'")
    assert(allMenus[menuName],"菜单'"..menuName.."'未定义")
    assert(type(parent)=="table" and parent.GetObjectType,"菜单"..menuName.."的父级不是框架或按钮")
    rematch.dragFrame:Hide()
...
function rematch.menus:Fill(menuFrame,subject)
    ...
    for index,info in ipairs(menu) do
        ...
        elseif info.title then
            -- title
            assert(index==1,"菜单标题必须是"..menuFrame.menuName.."中的第一个菜单条目")
