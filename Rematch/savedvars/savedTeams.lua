...
local function copyTeam(source,dest)
    assert(type(dest)=="table" and dest.teamID,"尝试复制到一个格式错误的团队: "..(source.teamID or "nil").."到"..(type(dest)=="table" and "nil" or "non-tabnle"))
    assert(validateTeamStructure(source),"尝试从格式错误的团队复制: "..(source.teamID or "nil").."到"..(dest.teamID or "nil"))
...
function rematch.savedTeams:Reset(teamID)
    assert(metaTeams[teamID],"重置必须是一个命名的metaTeam, 例如\"sideline\"或\"temporary\"")
    -- to reset a regular team, reset a sideline and assign it to the team
...
function rematch.savedTeams:Create()
    ...
    assert(sideline,"边线团队格式错误. 无法创建新团队.")
