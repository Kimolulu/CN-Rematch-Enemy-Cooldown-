...
function rematch.layout:Register(layout)
    ...
    assert(type(layout)=="table","布局格式: {mode=[number], view=[string], panels=[table]}")
    assert(layout.mode and layout.mode>=0 and layout.mode<=3,"布局模式必须是0-3之间的值.")
    assert(type(layout.view)=="string","布局必须具有命名视图, 例如'pets'.")
    assert(type(layout.width)=="number" and type(layout.height)=="number","布局必须具有宽度和高度.")
    assert(type(layout.panels)=="table" and #layout.panels>0,"布局必须至少定义一个面板.")
    ...
    for _,info in ipairs(layout.panels) do
        for i=1,11 do
            assert(info[i] and true,"布局面板格式: {parentKey, anchorPoint1, relativeTo1, relativePoint1, xoff1, yoff1, anchorPoint2, relativeTo2, relativePoint2, xoff2, yoff2}")
        end
        assert(info[1] and rematch.frame[info[1]],"布局面板nil parentKey在"..layoutName.."中.")
        assert(info[3] and rematch.frame[info[3]],"布局面板nil relativeTo1在"..layoutName.."中.")
        assert(info[8] and rematch.frame[info[8]],"布局面板nil relativeTo2在"..layoutName.."中.")
    end
...
function rematch.layout:GetLayout(layout)
    ...
    assert(false,"布局"..(layoutName or "nil").."不存在.")
end
